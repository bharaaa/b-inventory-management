...
export default function TotalValueView({ isOpen, onClose, items }) {
  ...
  return (
    <>
      ...
      {/* Grand Total Summary */}
      <div className="p-6 border-b border-[var(--border)] bg-white/30 backdrop-blur-md shadow-[inset_0_1px_0_rgba(255,255,255,0.6)] z-0">
        <div className="bg-white/50 backdrop-blur-xl rounded-2xl p-6 border border-[var(--border)] shadow-[inset_0_2px_4px_rgba(255,255,255,0.8),0_8px_24px_rgba(0,0,0,0.04)] flex items-center justify-between">
          <div>
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-white/10 backdrop-blur-sm">
        ...
        {sortedItems.length === 0 ? (
          <div className="text-center py-12">
            <div className="w-16 h-16 rounded-2xl bg-white/50 backdrop-blur-xl shadow-[inset_0_2px_4px_rgba(255,255,255,0.8)] border border-[var(--border)] flex items-center justify-center mx-auto mb-4 text-[var(--text-tertiary)]">
              <Package size={28} />
            </div>
            ...
            {sortedItems.map((item) => {
              ...
              return (
                <motion.div 
                  ...
                  className="bg-white/50 backdrop-blur-xl border border-[var(--border)] rounded-2xl p-4 flex flex-col gap-3 shadow-[0_2px_8px_rgba(0,0,0,0.04),inset_0_1px_0_rgba(255,255,255,0.8)] hover:-translate-y-0.5 hover:shadow-[0_8px_24px_rgba(0,0,0,0.06),inset_0_1px_0_rgba(255,255,255,0.9)] transition-all duration-300 group"
                >
                    ...
                    <div className="flex items-center gap-3">
                      {item.image_url ? (
                        <img
                          src={item.image_url}
                          alt={item.name}
                          className="w-11 h-11 rounded-xl object-cover bg-white/50 border border-[var(--border)] shadow-[inset_0_2px_4px_rgba(0,0,0,0.05)] shrink-0"
                        />
                      ) : (
                        <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-white/80 to-white/40 backdrop-blur-md border border-[var(--border)] shadow-[inset_0_2px_4px_rgba(255,255,255,0.8)] flex items-center justify-center shrink-0">
                  ...
                  <div className="flex items-center justify-between text-xs px-3.5 py-2.5 bg-white/40 backdrop-blur-md rounded-xl border border-[var(--border)] shadow-[inset_0_1px_2px_rgba(0,0,0,0.02)]">
